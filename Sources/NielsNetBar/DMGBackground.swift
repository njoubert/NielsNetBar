...
/// an arrow between the two icons, and what to expect on first launch (Login Item, Location
/// prompt). Drawn in code like the app icon; `build.sh dmg` renders it at 1× and 2× and
...
enum DMGBackground {
    ...
    static func draw() {
        ...
        let notes: [(String, String)] = [
            ...
            ("Location", "It asks for Location access because macOS only reveals the Wi-Fi network name (SSID) "
                + "to apps that have it. Nothing else is read and your location is never used."),
        ]
