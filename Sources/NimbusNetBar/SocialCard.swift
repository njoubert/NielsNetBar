...
enum SocialCard {
    ...
    private static let title = "Nimbus Net Bar"
    private static let pitch = "Live ↑/↓ throughput in the menu bar. Every interface's state, "
        + "addresses, link speed and Wi-Fi details in the dropdown."
    private static let slug = "github.com/njoubert/nimbus-net-bar"
    private static let accent = NSColor(srgbRed: 0.24, green: 0.855, blue: 0.843, alpha: 1)
    ...
    private static let shotAnchor = CGPoint(x: 0.5, y: 1)     // the top: bar readout, graph, totals
