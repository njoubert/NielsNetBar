...
final class NetworkWidget: NSObject, Widget {
    ...
    /// Same deal as the chart: a view so the two columns can be laid out against the menu's
    /// real width, which nothing knows until AppKit has measured it.
    ...
    /// USB adapters lists a dozen of them and none are why anyone opened the menu.
    /// `bool(forKey:)` is false when the key is unset, so hidden is the default for free.
    ...
    func buildMenu(into menu: NSMenu, host: WidgetController) {
        ...
        totals.setFrameSize(NSSize(width: NetworkWidget.chartWidth, height: TotalsView.height))
        rateRows = [:]
        ssidRows = [:]
        totalsRow = nil
        publicV4Row = nil
        ...
        if snapshot.interfaces.isEmpty {
            ...
            menu.addItem(host.disabled("No active interfaces"))
        }
    ...
    func settingsItems(host: WidgetController) -> [NSMenuItem] {
        ...
        return [inactiveItem, settings]
    }

    private var showInactive: Bool {
        UserDefaults.standard.bool(forKey: NetworkWidget.showInactiveDefaultsKey)
    }

    @objc private func toggleShowInactive(_ sender: NSMenuItem) {
        UserDefaults.standard.set(sender.state != .on, forKey: NetworkWidget.showInactiveDefaultsKey)
        // Clicking a menu item closes the menu, so the next open rebuilds with the new
        // setting. Rebuilding here would be both pointless and a flicker.
    }
    ...
    /// What clicking the headline copies. The view draws two columns, so there is no
    /// attributed title to fall back on.
