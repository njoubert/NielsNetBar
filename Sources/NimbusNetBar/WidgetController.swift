...
final class WidgetController: NSObject, NSMenuDelegate {
    ...
    /// the chart already forces, so no row can widen the menu by itself. **It has to move
    /// with that width** — left at 330 inside a 324 pt menu, every table row would push the
    /// menu wider, which is the one thing the fixed view width exists to prevent.
    ///
    ...
    static let tableTabStop: CGFloat = 276
